...
class ObjectParser(BaseParser):
    ...
    def detect_input(cls, content) -> bool:
        ...
        for item in items:
            if item is None:
                continue
            # The fallback parser, so any type an inspector can walk is accepted.
    ...
    def portable(name: str) -> str:
        """One module name or source path with the install's own directories taken off.

        Args:
            name: A dotted module name or a filesystem path.

        Returns:
            The part from ``custom_nodes`` onwards, or the last two segments where the
            text names no pack. A readout is saved into the workflow a user shares, so
            what it holds cannot be their drive letter and home directory.
        """
        if not name:
            return name
    ...
    def _introspect_object(cls, obj, logger=None) -> Optional[Dict]:
        ...
        module = cls.portable(type(obj).__module__)
    ...
    def _get_source_info(cls, obj) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                source_info["file"] = cls.portable(inspect.getfile(cls_type))
            except Exception:
                pass
