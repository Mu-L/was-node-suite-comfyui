"""
Base class for WAS Content Viewer parsers.

A parser declares ``PARSER_NAME`` and ``PARSER_PRIORITY``, and implements
``detect_input`` and ``handle_input`` for display, ``detect_output`` and
``parse_output`` for results.
"""
...
class BaseParser(ABC):
    ...
    #: Frontend view that draws what this parser produces. Defaults to ``PARSER_NAME``,
    #: which is right where a parser and a view share a name.
    PARSER_VIEW: str = None
